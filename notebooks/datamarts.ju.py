...
def write_clickhouse(df, table_name, main_col):
    df.write \
        .format("clickhouse") \
        ...
        .option("order_by", main_col) \
        .mode("overwrite") \
        .save()

def write_to_db(df, table_name, main_col):
...
# %%
df_product_sales = fact_sales.join(dim_products, fact_sales.product_id == dim_products.id, "inner")
...
write_to_db(products_top_10, "mart_products_top_10", "total_sold")
...
write_to_db(products_revenue_by_category, "mart_products_revenue", "total_revenue")
...
# %%
df_customer_sales = fact_sales.join(dim_customers, fact_sales.customer_id == dim_customers.id, "inner")
...
write_to_db(customers_by_country, "mart_customers_by_country", "customer_count")
...
time_average_order_size = df_time_sales.groupBy("sale_year", "sale_month") \
    ...
    .orderBy("sale_year", "sale_month")
write_to_db(time_average_order_size, "mart_time_average_order_size", "sale_year")
time_average_order_size.toPandas()


# %%
df_store_sales = fact_sales.join(dim_stores, fact_sales.store_id == dim_stores.id, "left")
...
write_to_db(suppliers_average_price, "mart_suppliers_average_price", "supplier_id")
...
suppliers_sale_by_country = df_supplier_sales.groupBy(dim_suppliers.country) \
    .agg(f.sum("sale_total_price").alias("total_revenue")) \
    .orderBy(f.desc("total_revenue"))
write_to_db(suppliers_sale_by_country, "mart_suppliers_sale_by_country", "total_revenue")
...
# %%
ratings_most_reviewed = products_rating.select("brand","name", f.col("total_reviews").alias("reviews")).orderBy(f.desc("reviews"))
...
write_to_db(ratings_correlation, "mart_ratings_rating_sales_correlation", "rating_sales_correlation")
